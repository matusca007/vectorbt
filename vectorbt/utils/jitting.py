...
def resolve_jitter_type(jitter: tp.Optional[tp.JitterLike] = None,
    ...
    * If `jitter` is a string, looks in `jitting.jitters` in `vectorbt._settings.settings`
...
def get_id_of_jitter_type(jitter_type: tp.Type[Jitter]) -> tp.Optional[tp.Hashable]:
    """Get id of a jitter type using `jitting.jitters` in `vectorbt._settings.settings`."""
    from vectorbt._settings import settings
    jitting_cfg = settings['jitting']
...
def resolve_jitted_option(option: tp.JittedOption = None) -> tp.KwargsLike:
    ...
    * dict: Use `option` as keyword arguments for jitting
    ...
    For defaults, see `jitting.option` in `vectorbt._settings.settings`."""
    from vectorbt._settings import settings
    jitting_cfg = settings['jitting']
