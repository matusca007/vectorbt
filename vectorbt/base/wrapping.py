...
It can also be indexed as a regular pandas object and integrates `vectorbt.base.grouping.Grouper`:
...
class ArrayWrapper(Configured, PandasIndexer):
    ...
    Tightly integrated with `vectorbt.base.grouping.Grouper` for grouping columns.

    If the underlying object is a Series, pass `[sr.name]` as `columns`.

    `**kwargs` are passed to `vectorbt.base.grouping.Grouper`.
